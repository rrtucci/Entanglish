...
\;,
\eeq
where $w_\alp$ for $\alp\in Z_{[0, n_\rvalp)}$
is a probability distribution
...
For $n=0$, define\footnote{Defining $K^{\alp}_{\rvx,\rvy}(0)=
...
Alternatively, for $n=0$, define $n_\rvalp$ to be equal to
the number of rows of $\rho_{\rvx, \rvy}$ and
...
\;
\eeq
for all $\alp$. Here $(\lam^\alp, \ket{\psi^\alp})$ for $\alp=0, 1, \ldots n_\rvalp-1$
are the eigenvalue, eigenvector pairs for $\rho_{\rvx, \rvy}$.
...
K^{\alp}_{\rvx,\rvy}(n+1)
=
{\rm regulate}\left(\frac{1}{2}\rho_{\rvx, \rvy}[\sum_\alp M^\alp_{\rvx, \rvy}(n)]^{-1}
M^\alp_{\rvx, \rvy}(n) + h.c.\right)
\;,
\eeq

\beq
E_{\rvx, \rvy}(n) = \frac{1}{2}\tr_{\rvx, \rvy} \left[\rho_{\rvx, \rvy}\Delta_{\rvx, \rvy}(n)\right]
\;,
\eeq
