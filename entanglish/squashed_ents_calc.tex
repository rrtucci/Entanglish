...
of
any pure or mixed quantum state $\rho$.
Our algorithm is a generalization
of the Arimoto-Blahut algorithm
 of classical
information theory,
where it is used to calculate channel capacities.
If the dimension of $\rho$ isn't too large,
one can calculate the squashed entanglement ``exactly"
using standard methods from Linear Algebra.
If the dimension of $\rho$ is large, then
one can calculate the squashed entanglement ``approximately"
 using a method that we call
  ``bootstrapped perturbation
theory" that is described in this paper.
In order to test
...
Executing the algorithm requires
...
Ref.\cite{Chr03}, by Christiandl and Winter in 2003.
...
by $\theta(|\lam_j|>\epsilon)$ for some small number
$\epsilon>0$. Let $\pi_0(\rho)=1-\pi_1(\rho)$,
so in $\pi_0(\rho)$, each $\lam_j$
in $D$ is replaced
by $\theta(|\lam_j|\leq \epsilon)$.
...
A different algorithm for doing
the same thing was proposed by Tucci
in Ref.\cite{Tuc01a} in 2001.
The algorithm presented below is significantly
better and simpler than the 2001 one. Both algorithms
are generalizations to the quantum regime
of the famous
Arimoto Blahut algorithm of classical information
theory, where it is used, for example, to calculate
channel capacities. See Ref.\cite{sq-wiki} for a
quick review of squashed entanglement. The section
of this paper entitled Introduction contains
additional references about it.

Consider a bipartite system
consisting of two parts labelled
by the random variable $\rvx$ and $\rvy$,
and
...
$\tr( \rho_{\rvx, \rvy}^2 - \rho_{\rvx, \rvy}) = 0$) then\footnote{ The factor of 1/2 in
...
Doh, the fact that
squashed entanglement reduces to
the pure state entanglement
definition, which was invented much earlier,
 was one of the
original motivations for Tucci for defining
squashed entanglement.}
...
Below, we will use the symbols $K_{\rvx, \rvy}^\alp$,
$K_{\rvx,\rvy}$ and
$R_{\rvx, \rvy}^\alp$ to denote
\beq
K_{\rvx, \rvy}^\alp = w^\alp \rho_{\rvx, \rvy}^\alp
\;,
\eeq

\beq
 K_{\rvx, \rvy} =
\sum_\alp K_{\rvx, \rvy}^\alp =
 \rho_{\rvx, \rvy}
\;,
\eeq
and
...
\beq
R_{\rvx, \rvy}^\alp=
\frac{K_{\rvx}^\alp K_{\rvy}^\alp}{w^\alp}
\;.
\eeq
...
squashed entanglement of either pure or mixed states
consists of the two steps of Eqs.(\ref{eq-ab-algo})\footnote{The notation used in Eqs.(\ref{eq-ab-algo})
is identical to that used in Ref.\cite{Tuc01a},
except that in Eqs.(\ref{eq-ab-algo})
we introduce
a new operator $\cale^{(n)}_{\rvx, \rvy}$
which is related to the operator
$\Delta^{(n)}_{\rvx, \rvy}$ used in
Ref.\cite{Tuc01a}
by the following equation:
$
\cale^{(n)}_{\rvx, \rvy}=
K_{\rvx, \rvy}\Delta^{(n)}_{\rvx, \rvy}
$.}
iterated for $n=0, 1, 2, \ldots, \nab-1$,
where the number of Arimoto Blahut steps $\nab$
can be fixed a priori
or decided on the fly. If on the fly, one can
stop when the change in $E_{\rvx, \rvy}$
between  consecutive steps
becomes smaller than a pre-decided constant $\epsilon>0$.
...
K_{\rvx, \rvy}^{\alp (n)}
\left(
\log K_{\rvx, \rvy}^{\alp (n)}
- \log R_{\rvx, \rvy}^{\alp (n)}
\right)
\;,
\label{eq-ab-algo-a}
\eeq
...
\right)
\pi_1(K_{\rvx, \rvy})
\right]-\pi_0(K_{\rvx, \rvy})
\;.
\label{eq-ab-algo-b}
\eeq
\end{subequations}

In the algorithm of Eqs.(\ref{eq-ab-algo}),
the entanglement $E_{\rvx, \rvy}$ (which is a
non-negative number)
is computed as the limit
of the trace of the
entanglement operator $\cale^{(n)}_{\rvx, \rvy}$:

\beq
E_{\rvx, \rvy} = \lim_{n\rarrow \infty}
\tr_{\rvx, \rvy} \cale^{(n)}_{\rvx, \rvy}
\approx
\tr_{\rvx, \rvy} \cale^{(\nab-1)}_{\rvx, \rvy}
\;.
\eeq

Here is some intuition about the use of
projectors $
\pi_0(K_{\rvx, \rvy}), \pi_1(K_{\rvx, \rvy})$ in Eqs.(\ref{eq-ab-algo}).
When $K_{\rvx, \rvy}$ has zero
eigenvalues, its inverse doesn't exist, but its pseudo
inverse does.
The argument of the exp() in Eq.(\ref{eq-ab-algo-b})
is sandwitched between
two $\pi_1(K_{\rvx, \rvy})$.
Each zero eigenvalue of
$K_{\rvx, \rvy}$,
when exponentiated becomes a 1 because $\exp(0)=1$,
and each of those ones along the diagonal
is cancelled by
subtracting a $\pi_0(K_{\rvx, \rvy})$.
The net effect of using
the projectors $\pi_0(K_{\rvx, \rvy})$
and $\pi_1(K_{\rvx, \rvy})$
in Eq.(\ref{eq-ab-algo-b}) is to produce
 a $K_{\rvx, \rvy}^{\alp (n+1)}$
that satisfies
...
\label{eq-constraint}
\eeq
Eq.(\ref{eq-constraint}) is a good constraint to impose because
it must be satisfied as $n\rarrow\infty$.


Of course, if $K_{\rvx, \rvy}$ doesn't have any
zero eigenvalues (i.e., is non-singular),
then the pseudo-inverse becomes the usual
inverse, $\pi_0(K_{\rvx, \rvy})=0$,
and $\pi_1(K_{\rvx, \rvy})=1$.
...
So now instead of getting expression
...
from $\rho$,
and such that esys($\rho_0$) is easy to
...
We have presented an algorithm given by Eqs.(\ref{eq-ab-algo})
for calculating the squashed
entanglement of any pure or mixed state $\rho$.
For pure states, however,  it is
simpler and gives the same answer
if one just evaluates the von Neumann entropy
of $\rho$ traced over either of the two
subsystems which characterize the bipartite
entanglement. Whether one uses Eqs.(\ref{eq-ab-algo})
or the von Neumann entropy
of a partial trace of $\rho$,
various eigen-systems of Hermitian matrices will
have to be calculated along the way to
obtain exp(), log(), pinv() etc. of
various  matrices.
If the dimension of $\rho$ isn't too large,
one can calculate those
eigen-systems ``exactly"
using standard methods from Linear Algebra.
If the dimension of $\rho$ is too large
for that, then
one can use the ``bootstrapped perturbation
theory" method described in this paper.
...
\bibitem{Tuc99} Tucci, Robert R. (1999). ``Quantum Entanglement and Conditional Information Transmission". arXiv:quant-ph/9909041.
\bibitem{Tuc00a} Tucci, Robert R. (2000). ``Separability of Density Matrices and Conditional Information Transmission". arXiv:quant-ph/0005119.
\bibitem{Tuc00b} Tucci, Robert R. (2000). ``Entanglement of Formation and Conditional Information Transmission". arXiv:quant-ph/0010041.
\bibitem{Tuc01a} Tucci, Robert R. (2001). ``Relaxation Method for Calculating Quantum Entanglement". arXiv:quant-ph/0101123.
\bibitem{Tuc01b} Tucci, Robert R. (2001). ``Entanglement of Bell Mixtures of Two Qubits". arXiv:quant-ph/0103040.
\bibitem{Tuc02} Tucci, Robert R. (2002). ``Entanglement of Distillation and Conditional Mutual Information". arXiv:quant-ph/0202144

\bibitem{Chr03} Matthias Christandl, Andreas Winter (2003). ````Squashed Entanglement": An Additive Entanglement Measure". Journal of Mathematical Physics. 45 (3): 829–840. arXiv:quant-ph/0308088
...
\bibitem{sep-proof2}
 Brandao, F., Christandl, M., Yard, J. (September 2011). ``Faithful Squashed Entanglement". Communications in Mathematical Physics. 306 (3): 805–830. arXiv:1010.1750

\bibitem{pert-wiki}
https://en.wikipedia.org/wiki/Perturbation\_theory\_(quantum\_mechanics)

\bibitem{cvit}
P. Cvitanović, {\it Group Theory: Birdtracks, Lie's, and Exceptional Groups}, Princeton University Press, Princeton 2008, available online at http://birdtracks.eu/

\bibitem{wiki-hypergeo}
https://en.wikipedia.org/wiki/Hypergeometric\_distribution
